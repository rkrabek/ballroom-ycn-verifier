...
print "done loading"
# competitors['RobinGanek'] = individual_res5.get_ycn("Ganek","Robin")
...
def check_eligibility_pts(name, level, style, dance):
    def pt_lookup(level):
        try:
            ...
            return 0
    l_index = level_list.index(level)
    curr_lvl_pts = pt_lookup(level)
    if l_index > 5:
        return (True, 0)
    else:
        pt_trickle = (curr_lvl_pts + pt_lookup(level_list[l_index+1]))
        if l_index < 5:
            if pt_trickle < 7:
                if (pt_lookup(level_list[l_index+2]) != 0):
...
def get_entry_style(entry, dances):
    ...
    if len(style_try) == 0:
        ...
        style = style_try[0][0]
        # print style
    dances = [dance_map[style][x] for x in dances]
    return (style, dances)
...
for post in entries_post("mit"):
    ...
    if len(names) != 0:
        ...
        event = post.text
        level = get_entry_level(event)
        dances_abrv = get_entry_dances(event)
        sty_dan = get_entry_style(event, dances_abrv)
        style = sty_dan[0]
        dances = sty_dan[1]
        # print '\n'+ event
        # print level
